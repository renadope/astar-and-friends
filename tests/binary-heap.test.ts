...
describe('binary heap', () => {
  const orderedPositiveValues = Array.from({ length: 10000 }).map((_, index) =>
    makeNodeWithValueAsPriorityAutoID(index + 1)
  );
  const orderedNegativeValues = Array.from({ length: 100 }).map((_, index) =>
    makeNodeWithValueAsPriorityAutoID(-index - 1)
  );
  ...
  describe('minHeap', () => {
    ...
    it('should handle duplicate priorities', () => {
      minHeap.insertAll([5, 5, 1, 1].map((val) => makeNodeWithValueAsPriorityAutoID(val)));
    ...
    it('should handle random large dataset', () => {
      const randomValues = fisherYates(
        Array.from({ length: 10000 }, () => Math.floor(Math.random() * 10000)).map((val) =>
          makeNodeWithValueAsPriorityAutoID(val)
        )
      );
    ...
    it('should update priority and maintain heap property', () => {
      const newVal =
        orderedPositiveValues[orderedPositiveValues.length - 1].value *
  ...
  describe('maxHeap', () => {
    ...
    it('should handle duplicate priorities', () => {
      maxHeap.insertAll([5, 5, 1, 1].map((val) => makeNodeWithValueAsPriorityAutoID(val)));
