  ...
  private heapifyDownFrom(index: number): void {
    while (this.hasLeftChild(index)) {
      ...
      if (this.hasRightChild(index)) {
        ...
        const compare = this.compare(this.heap[rightIndex], this.heap[betterChildIndex]);
        if (compare < 0) {
          betterChildIndex = rightIndex;
        }
      }
      const compareToParent = this.compare(this.heap[index], this.heap[betterChildIndex]);
