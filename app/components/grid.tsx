                    ...
                    <div className="mt-2 2xs:mt-3 sm:mt-4 flex flex-wrap gap-1 2xs:gap-1.5 sm:gap-2 justify-center">
                        {["empty", "wall", "visited", "frontier", "path", "start", "goal", "ghost"].map(state => (
                            <div key={state}
                                ...
                                <span
                                    className="text-xs 2xs:text-xs sm:text-sm text-slate-700 capitalize">{state}</span>
                            </div>
                        ))}
                    </div>
                )}

                <Dialog open={isValidClickedCell && !hasAStarData && cellSelectionState === 'inactive'}
                ...
                >
                    <DialogContent>
                        <DialogHeader>
                            <DialogTitle>
                                Edit
                                Cell: {!isNullOrUndefined(clickedCell) ? `(${clickedCell[0]},${clickedCell[1]})` : ''}
                            </DialogTitle>
                            <DialogDescription>
                                Configure the weight of this cell.
                            </DialogDescription>
                        </DialogHeader>
                        <Input
                            type="number"
