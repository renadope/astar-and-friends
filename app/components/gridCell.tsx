...
function BasicCellInfo({
  ...
  return (
    <div
      className={cn(
        'flex flex-col gap-0.5 items-center w-full h-full justify-center group',
        className
      )}
      {...props}
    >
      <p
        className={`2xs:hidden sm:block text-xs md:text-sm lg:text-lg ${textColors[cell.state] || 'text-slate-500'}
...
export default function GridCell({
  ...
  const bestFrontier =
    cell.state === 'frontier' &&
    !isNullOrUndefined(posUpNext) &&
    r === posUpNext[0] &&
    c === posUpNext[1];

  const hasVisitedReconstructedPath = !isNullOrUndefined(allReconstructedPathsCache)
    ? allReconstructedPathsCache.has(stringifyPos(r, c))
    : false;
  const canGhost =
    !isPlaying && currentTimelineIndex >= timeline.length - 1 && hasVisitedReconstructedPath;
  const canPaintCell =
    isPainting &&
    !isNullOrUndefined(paintingWeight) &&
    !hasAStarData &&
  ...
  return (
    <div
      ...
      className={`
        ...
        ${!isNullOrUndefined(aStarData?.fallBack) && isSamePos([r, c], goalPos) && isLastStep ? 'opacity-65 blur-[1px] -rotate-3 ' : ''}
        `}
      onClick={() => {
    ...
    >
      ...
      {isCurrentStep && isLastStep && (
        <div className="animate-[wiggle_1s_ease-in-out_infinite] absolute top-0 left-0 text-lg">
          🏁
        </div>
      )}

      {(cell.state === 'path' || isCurrentStep) && (
        <div className="absolute inset-0 rounded-md bg-white opacity-0 group-hover:opacity-20  pointer-events-none"></div>
      )}

      {cell.f !== undefined && (
        <div className="hidden lg:block absolute top-0 right-0 text-xs bg-slate-800 text-white px-1.5 py-0.5 rounded-bl-lg rounded-tr-lg font-bold shadow-lg">
          f:{cell.f.toFixed(1)}
        </div>
      )}
    </div>
