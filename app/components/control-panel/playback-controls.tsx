...
export function PlaybackControls({ className, ...props }: ComponentPropsWithoutRef<'div'>) {
  ...
  const handleJumpToPathStart = useCallback(
    () => dispatch({ type: 'JUMP_TO_PATH_START' }),
    [dispatch]
  );
...
function PlaybackStatusIndicator({ className, ...props }: ComponentPropsWithoutRef<'div'>) {
  ...
  const timeline = state.timeline === 'snapshot' ? state.snapshotTimeline : state.granularTimeline;
  return (
    <div className={cn(`flex items-center justify-center gap-3 pt-2`, className)} {...props}>
      <div
      ...
      >
        ...
        <div className="text-sm font-medium">
          {currentTimelineIndex >= 0 ? (
            ...
            <span className={hasNoAStarData ? 'text-gray-700' : 'text-green-700'}>
              {!hasNoAStarData ? 'Ready to start' : 'Waiting...'}
            </span>
          )}
        </div>
      </div>
