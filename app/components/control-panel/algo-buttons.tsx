...
function RunAStarButton({
  configChanged,
  className,
  onClick,
  ...props
}: ComponentPropsWithoutRef<'button'> & { configChanged: boolean }) {
  return (
    <button
      className={cn(
        `group relative px-2 2xs:px-3 sm:px-4 lg:px-6 py-2 2xs:py-3 bg-gradient-to-r text-white rounded-lg shadow-lg transition-all duration-300
       flex items-center justify-center gap-1 2xs:gap-1.5 font-bold text-xs 2xs:text-sm overflow-hidden min-w-0 flex-shrink ${
         configChanged
...
function CalculatePathButton({
  ...
  return (
    <button
      className={cn(
        `group relative px-2 2xs:px-3 sm:px-4 py-2 border-2 border-gray-300 bg-white hover:bg-gray-50 text-gray-700 hover:text-gray-900 rounded-xl shadow-sm transition-all duration-300 flex items-center justify-center gap-1.5 2xs:gap-2 font-medium hover:scale-105 hover:-translate-y-0.5 hover:shadow-md min-w-0 flex-shrink ${
          configChanged ? 'border-2 border-blue-400 bg-blue-50 text-blue-700 shadow-blue-100' : ''
        }`,
        className
      )}
      onClick={onClick}
...
function ResetButton({
  ...
  return (
    <button
      disabled={disabled}
      className={cn(
        `group px-2 2xs:px-3 sm:px-4 py-2 2xs:py-3 bg-gradient-to-br
...
export function GenerateGridButton({
  ...
  return (
    <button
      className={cn(
        `group px-2 2xs:px-3 sm:px-4 lg:px-6 py-2 2xs:py-3 bg-gradient-to-r from-sky-500 to-sky-600 hover:from-sky-600 hover:to-sky-700 
      text-white rounded-lg shadow-lg hover:shadow-xl transition-all duration-300 flex items-center
       justify-center gap-1 2xs:gap-1.5 font-semibold text-xs 2xs:text-sm transform hover:scale-105 hover:-translate-y-0.5 min-w-0 flex-shrink`,
        className
      )}
      onClick={onClick}
      {...props}
